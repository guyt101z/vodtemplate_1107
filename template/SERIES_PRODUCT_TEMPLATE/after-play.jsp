...
<%
	...
	String playNextURL = "http://" + afterplayip + "/vodtemplate/template/SERIES_PRODUCT_TEMPLATE/checkFlags.jsp?"+urlParam+"&cat="+cat+"&seriesid="+seriesId+"&seriesproductid="+nextSeriesProductId+"&scheduleid="+nextSeriesScheduleId+"&seriesindex="+seriesIndex;
	String seriesDetailPageURL = "http://" + afterplayip + "/vodtemplate/template/SERIES_PRODUCT_TEMPLATE/seriesProgramMain.jsp?"+urlParam+"&cat="+cat+"&seriesid="+seriesId+"&seriesproductid="+seriesProductId+"&seriesindex="+seriesIndex;
		...
		if (message!=null) {
			...
			%>
		 	if ("<%=seriesId%>" != "" && "<%=seriesId%>" != "null"){ 
				...
				window.location.href="grid_view.jsp?catId=<%=catId%>&lang=<%=language%>&version=<%=version%>&libId=<%=libId%>&styleId=<%=styleId%>";
			}
			<%
			} else {
