    ...
    <script language="javascript" src="file:////javascript/iptv.js"></script>
  	...
  	<script language="javascript">
		...
		var seriesIndex = "<%=seriesIndex%>";
                urlparamAppend="&seriesid="+seriesId+"&seriesproductid="+seriesProductId+"&seriesindex="+seriesIndex;
  		var globalSVODLock = iptvGetNonVolatileData("g_svl");
			...
			if (l_is_locked == "true" || (restrictedSVODLock != "N" && l_is_restricted == "true") || globalSVODLock == "Y") {
				window.parent.location.href="thankyouwithpin.jsp?verified=false&playonly=false&scheduleid=<%=scheduleId%>&<%=urlParam%>"+urlparamAppend;
			}
		...
		function keyEvent(keyCode) {
			if(keyCode == KEY_ENTER){
				if(globalSVODLock == "Y"){
					...
					window.parent.location.href="qapi/getsvod.jsp?verified=true&playonly=false&scheduleid=<%=scheduleId%>&<%=urlParam%>"+urlparamAppend;
				}
			} else if(keyCode == KEY_BACK){
				window.parent.location.href="seriesProgramMain.jsp?playonly=false&scheduleid=<%=scheduleId%>&<%=urlParam%>"+urlparamAppend;
			} else if(keyCode == KEY_GREEN){
				window.parent.location.href="class.jsp?playonly=false&scheduleid=<%=scheduleId%>&"+ urlparam+urlparamAppend;
